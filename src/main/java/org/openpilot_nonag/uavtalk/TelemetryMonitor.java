...
public class TelemetryMonitor extends Observable {
    ...
    static final Logger logger = LogManager.getLogger(TelemetryMonitor.class.getName());
    ...
    public synchronized void startRetrievingObjects() throws IOException {
        logger.debug("Start retrieving objects");
        ...
        // Start retrieving
        logger.debug("Starting to retrieve meta and settings objects from the autopilot ("
                + queue.size() + " objects)");
        retrieveNextObject();
    }
    ...
    public void stopRetrievingObjects() {
        logger.debug("Stop retrieving objects");
        queue.clear();
    }
    ...
    public synchronized void retrieveNextObject() throws IOException {
        ...
        if (queue.isEmpty()) {
            ...
            if (DEBUG) logger.debug("All objects retrieved: Connected Successfully");
        ...
        if (DEBUG)
            logger.debug("Retrieving object: " + obj.getName());
    ...
    public synchronized void transactionCompleted(UAVObject obj, boolean success)
            throws IOException {
        if (DEBUG)
            logger.debug("transactionCompleted.  Status: " + success);
    ...
    public synchronized void processStatsUpdates() throws IOException {
        ...
        if (DEBUG)
            logger.debug("GCS: " + statusField.getValue() + " Flight: "
                    + flightStatsObj.getField("Status").getValue());
        ...
        if (oldStatus.compareTo("Disconnected") == 0) {
            ...
            if (((String) flightStatsObj.getField("Status").getValue())
                ...
                if (DEBUG)
                    logger.debug("Connected" + statusField.toString());
            }
        ...
        if (!gcsConnected || !flightConnected) {
            if (DEBUG)
                logger.debug("Sending gcs status");
            gcsStatsObj.updated();
        }
        ...
        if (gcsConnected && gcsStatusChanged) {
            if (DEBUG)
                logger.debug("Connection with the autopilot established");
        ...
        if (gcsDisconnected && gcsStatusChanged) {
            if (DEBUG)
                logger.debug("Trying to connect to the autopilot");
            setPeriod(STATS_CONNECT_PERIOD_MS);
            connected = false;
            objects_updated = false;
            setChanged();
        ...
        if (DEBUG)
            logger.debug("processStatsUpdates() - before notify");
        notifyObservers();
        if (DEBUG)
            logger.debug("processStatsUpdates() - after notify");
    }
    ...
    private final Observer firmwareIapUpdated = new Observer() {
        @Override
        public void update(Observable observable, Object data) {
            if (DEBUG) logger.debug("Received firmware IAP Updated message");
            ...
            if (description == null || description.getNumElements() < 100) {
                ...
                logger.info("Failed to determine UAVO set");
            } else {
                ...
                if (telemService.loadUavobjects(jarName, objMngr)) {
                    ...
                    logger.info("Failed to load UAVO set: " + jarName);
                }
            }
