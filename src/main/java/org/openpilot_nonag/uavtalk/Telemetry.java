...
public class Telemetry {
        ...
        while (li.hasNext()) {
            ...
            if (objinfo.updatePeriodMs > 0) {
                ...
                if (objinfo.timeToNextUpdateMs <= 0) {
                    ...
                    if (DEBUG) logger.debug("Manual update: " + objinfo.obj.getName());
    ...
    public class ObjectUpdateHandler {
        ...
        void enqueueObjectUpdates(UAVObject obj, int event,
            ...
            if (DEBUG) logger.debug("Enqueing update " + obj.getName() + " event " + event);
            ...
            synchronized(objQueue) {
                if (objQueue.contains(objInfo)) {
                    if (WARN) logger.trace("Found previously scheduled queue element: " + objInfo.obj.getName());
                } else {
        ...
        public boolean removeActivatedQueue(ObjectQueueInfo objInfo) {
            synchronized(objQueue) {
                if (objQueue.remove(objInfo)) {
                    if (WARN) logger.trace("Unable to find queue element to remove");
                    return false;
                }
            }
            return true;
    ...
    class ObjectRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (DEBUG) logger.debug("Object transaction running.  Event:" + objInfo.event);
            ...
            if (((String) gcsStatsObj.getField("Status").getValue()).compareTo("Connected") != 0) {
                if ((objInfo.obj.getObjID() != objMngr.getObject("GCSTelemetryStats").getObjID()) &&
                    ...
                    if (DEBUG) logger.debug("transactionCompleted(false) due to receiving object not GCSTelemetryStats while not connected.");
            ...
            if (objInfo.event != EV_UNPACKED && ((objInfo.event != EV_UPDATED_PERIODIC) || (updateMode != UAVObject.UpdateMode.UPDATEMODE_THROTTLED))) {
                ...
                synchronized (transInfo) {
                    ...
                    if (transPending && newTransactionPending) {
                        if (WARN) logger.trace("Postponing transaction for" + newTrans.obj.getName() + " existing transaction for " + transInfo.obj.getName());
                        //handler.postDelayed(this, 100);
                        return;
                    }

                    if (DEBUG) logger.debug("Process Object transaction for " + newTrans.obj.getName());
                    ...
                    try {
                        ...
                        if (newTrans.objRequest) {
                            if (DEBUG) logger.debug("Sending object request " + newTrans.obj.getName());
                            utalk.sendObjectRequest(newTrans.obj, newTrans.allInstances);
                        } else {
                            if (DEBUG) logger.debug("Sending object " + newTrans.obj.getName());
                            utalk.sendObject(newTrans.obj, newTrans.acked, newTrans.allInstances);
                        }

                    } catch (IOException e) {
                        if (ERROR) logger.error("Unable to send UAVTalk message");
                        e.printStackTrace();
                    }
    ...
    final Runnable transactionTimeout = new Runnable() {
        ...
        public void run() {
            ...
            synchronized (transInfo) {
                ...
                if (!transPending) {
                    if (WARN) logger.trace("Transaction completed but timeout still called.  Probable race condition");
                    return;
                }

                if (DEBUG) logger.debug("Telemetry: transaction timeout.");
                ...
                if (transInfo.retriesRemaining > 0) {
                    ...
                    if (DEBUG) logger.debug("Sending object request");

                    try {
                        // Execute transaction by sending the appropriate UAVTalk command
                        if (transInfo.objRequest) {
                            if (DEBUG) logger.debug("Sending object request" + transInfo.obj.getName());
                            utalk.sendObjectRequest(transInfo.obj, transInfo.allInstances);
                        } else {
                            if (DEBUG) logger.debug("Sending object " + transInfo.obj.getName());
                    ...
                    if (ERROR) logger.error("Transaction failed for: " + transInfo.obj.getName());
    ...
    private void transactionCompleted(UAVObject obj, boolean result) {
        ...
        if (DEBUG) logger.debug("UAVTalk transactionCompleted");

        // Check if there is a pending transaction and the objects match
        synchronized(transInfo) {
            if (transPending && transInfo.obj.getObjID() == obj.getObjID()) {
                if (DEBUG) logger.debug("Telemetry: transaction completed for " + obj.getName());
                ...
                if (ERROR) logger.error("Error: received a transaction completed when did not expect it. " + obj.getName());
                transPending = false;
            }
        }
    }
