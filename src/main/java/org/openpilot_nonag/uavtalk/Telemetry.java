...
public class Telemetry {
	...
	final Observer updatedAutoObserver = new Observer() {
		...
		public void update(Observable observable, Object data) {
            ...
            logger.debug("***** updatedAutoObserver");
		}
	};
	...
	final Observer updatedManualObserver = new Observer() {
		...
		public void update(Observable observable, Object data) {
            ...
            logger.debug("***** updatedManualObserver");
		}
	};
	...
	final Observer updatedRequestedObserver = new Observer() {
		...
		public void update(Observable observable, Object data) {
            ...
            logger.debug("***** updatedRequestedObserver");
		}
	};
		...
		public void run() {
			...
			if (((String) gcsStatsObj.getField("Status").getValue()).compareTo("Connected") != 0) {
				if (objInfo.obj.getObjID() != objMngr.getObject("GCSTelemetryStats").getObjID()) {
					if (DEBUG)
						logger.debug("transactionCompleted(false) due to receiving object not GCSTelemetryStats while not connected.");
			...
			if (objInfo.event != EV_UNPACKED) {
				...
				synchronized (transInfo) {
					...
					if (transPending && newTransactionPending) {
						logger.warn( "Postponing transaction for" + newTrans.obj.getName() + " existing transaction for " + transInfo.obj.getName());
	...
	final Runnable transactionTimeout = new Runnable() {
		...
		public void run() {
			...
			synchronized (transInfo) {
				...
				if (!transPending) {
					logger.warn("Transaction completed but timeout still called.  Probable race condition");
					return;
				}
	...
	private void transactionCompleted(UAVObject obj, boolean result) {
		...
		synchronized(transInfo) {
			if (transPending && transInfo.obj.getObjID() == obj.getObjID()) {
				...
				logger.error("Error: received a transaction completed when did not expect it. " + obj.getName());
				transPending = false;
			}
		}
	}
